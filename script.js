...
const debugBuildId = "20260612-design-2";
...
if (debugMode) {
  ...
  diagnosticPanel.classList.remove("is-hidden");
  diagnosticPanel.setAttribute("aria-hidden", "false");
}

const failureMessageMap = {
  closed_before_reward: "還差一點，看完廣告就能打開答案。",
  no_fill: "目前暫時無法開啟廣告，請稍後再試。",
...
function updateRewardStatus(message, progress, meta = "") {
  ...
  rewardMeta.textContent = debugMode ? meta : "";
  appendDiagnostic(`${message}${meta ? ` | ${meta}` : ""}`);
}

function showAnswer() {
  rewardPanel.classList.remove("is-visible");
...
async function simulateRewardFlow() {
  ...
  updateRewardStatus("請稍候...", 62);
  await wait(1400);

  updateRewardStatus("答案已解鎖。", 100);
  await wait(500);
